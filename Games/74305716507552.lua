...
function equiptool()
    if not player.Character:FindFirstChild("FreeSlap") then
        player.Backpack:FindFirstChild("FreeSlap").Parent = player.Character
    end
end
...
Tabs.Troll:AddButton({
    Title = "Get tool",
    Callback = function()
        if not player.Backpack:FindFirstChild("FreeSlap") or player.Character:FindFirstChild("FreeSlap") then
            player.Character.HumanoidRootPart.CFrame = CFrame.new(132, 955, -42)
			task.wait(.1)
...
Tabs.Troll:AddButton({
    ...
    Callback = function()
        ...
        player.Character:WaitForChild("FreeSlap"):WaitForChild("Event"):FireServer(unpack(args))
    end
})
